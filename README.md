...
This repository also includes a compact **web metabrowser** (`Busca Aérea`) built with Next.js for cash/award comparison, price alerts, and admin health checks. The Claude/Codex/OpenCode plugin toolkit remains intact.
...
```bash
cp .env.example .env
# fill DATABASE_URL, AUTH_SECRET, and optional provider keys

docker compose up -d db
npm install
npx prisma migrate deploy
npm run prisma:seed   # optional, needs SEED_ADMIN_EMAIL + SEED_ADMIN_PASSWORD
npm run dev
```
...
Airport coordinate data comes from OpenFlights ([openflights.org/data.php](https://openflights.org/data.php)) under the **ODbL** license.
...
See [docs/coolify-deploy.md](docs/coolify-deploy.md). Architecture notes: [docs/app-architecture.md](docs/app-architecture.md).

Schedule price alerts:

```bash
curl -X POST "$APP_URL/api/cron/alerts" -H "Authorization: Bearer $CRON_SECRET"
```
